...
!function( $ ) {
	...
	var Datetimepicker = function(element, options) {
		...
		// 用以下代码替换
		this.component = this.element.is('.date') ? this.element.find('.fa-calendar').parent() : false;  
		//此代码有问题，导致输入框中的日期虽然被删除，但是又弹出了日期选择窗口，并且日期选择窗口不能被翻页
		//this.componentReset = this.element.is('.date') ? this.element.find('.date-reset').parent() : false;  
		// 用以下代码替换
		this.componentReset = this.element.is('.date') ? this.element.find('.fa-times').parent() : false;
	...
	var DPGlobal = {
		...
		headTemplate: '<thead>'+
							'<tr>'+
								'<th class="prev" title="左翻"><i class="fa fa-angle-left"/></th>'+
								'<th colspan="5" class="switch" title="点击更换年月"></th>'+
								'<th class="next" title="右翻"><i class="fa fa-angle-right"/></th>'+
							'</tr>'+
						'</thead>',
